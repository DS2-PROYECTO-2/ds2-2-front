...
describe('Register - Comportamiento de Éxito', () => {
  beforeEach(() => {
    mockFetch.mockClear();
  ...
  it('cierra el modal cuando se hace clic en el botón', async () => {
    // Mock de respuesta exitosa de la API
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
    ...
    // Verificar que el modal se cierra (no debe estar visible)
    await waitFor(() => {
  ...
  it('muestra el mensaje correcto del backend en el modal', async () => {
    const customMessage = 'Usuario creado exitosamente. Revisa tu email para verificar tu cuenta.';
    
    // Mock de respuesta exitosa de la API con mensaje personalizado
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
    ...
    // Verificar que se muestra el mensaje personalizado del backend
    await waitFor(() => {
      expect(screen.getByText('¡Registro Exitoso!')).toBeInTheDocument();
