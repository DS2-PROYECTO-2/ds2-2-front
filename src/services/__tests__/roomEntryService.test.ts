...
describe('roomEntryService', () => {
  it('getMyEntries mapea lista', async () => {
    vi.spyOn(api.apiClient, 'get').mockResolvedValueOnce({ entries: [{ id: 1, room: 2, entry_time: '2024-01-01T00:00:00Z' }] } as any)
  ...
  it('getMyActiveEntry mapea activo', async () => {
    vi.spyOn(api.apiClient, 'get').mockResolvedValueOnce({ has_active_entry: true, active_entry: { id: 3, room: 9, entry_time: '2024-01-01T00:00:00Z' } } as any)
  ...
  it('createEntry usa post', async () => {
    const spy = vi.spyOn(api.apiClient, 'post').mockResolvedValueOnce({ ok: true } as any)
    await createEntry(5, 'n')
    expect(spy).toHaveBeenCalled()
  })

  it('exitEntry usa patch ruta 1', async () => {
    const spy = vi.spyOn(api.apiClient, 'patch').mockResolvedValueOnce({ ok: true } as any)
    await exitEntry(10, 'bye')
    expect(spy).toHaveBeenCalled()
  })

  it('getAllEntries compone URL con filtros', async () => {
    const spy = vi.spyOn(api.apiClient, 'get').mockResolvedValueOnce({ entries: [] } as any)
